...
//Rare exceptions to indicate a bug in the _qeahelper code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_qeahelper = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_qeahelper]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.qeahelper.module"
