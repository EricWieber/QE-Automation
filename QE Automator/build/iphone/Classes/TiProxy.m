...
//Rare exceptions to indicate a bug in the _qeautomator code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_qeautomator = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_qeautomator]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.qeautomator.module"
